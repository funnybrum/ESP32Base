...
/**
 * Support integration with an InfluxDB. Configurable collection interval, custom implementation
 * that dictates when data collection can start, custom code to collect the required data points.
 * 
 * Configurations can be performed through a browser. Settings include time between collecting
 * data points, time between data push attempts, address of the InfluxDB server.
 * 
 * This is implementation with in-memory buffer. Data is collected in memory and pushed with
 * timestamp for the moment when it was collected.
 * 
 * Check https://github.com/funnybrum/ESP32Blank for example implementation.
*/
class InfluxDBCollectorBase {
    public:
        ...
        // Will be called on each push. Can be used for tracking when a push has been performed.
        virtual void onPush() = 0;

        // Allow custom logic to force the push. If the call to shouldPush returns true data will
        // be pushed ASAP, ignoring the default push interval in the settings.
        virtual bool shouldPush() = 0;

        InfluxDBCollectorBase(Logger* _logger,
                              WiFiManager* _wifi,
        ...
        void loop() {
            ...
            // the InfluxDB server the InfluxDBCollector will keep a constant loop of disconnet,
            // connect, http call events untill the data is pushed. The impact - failrue to push
            // the data can bring down the remaining functionality.
